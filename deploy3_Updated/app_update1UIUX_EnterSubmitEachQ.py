...
async def scoring_short_essay_questions(QUESTION, CHECKING_ANSWER, USER_ANSWER):
    async with AsyncOpenAI(api_key=OPENAI_API_KEY) as client:
        prompt = f"""
                ...
                You are provided with 1 QUESTION and 1 CHECKING ANSWER for that QUESTION. After USER enters the USER ANSWER, you will check USER ANSWER with CHECKING ANSWER. 
...
def create_quiz_interface(file_path, sheet_name_mc, sheet_name_essay, output_dir):
    ...
    with gr.Blocks() as demo:
        ...
        async def handle_user_input(user_answer, state):
            ...
            if is_quiz_finished:
                ...
                return (
                    ...
                    gr.update(visible=False),
                    gr.update(visible=False),
                    gr.update(value=quiz_result, visible=True),
                    gr.update(value=f"{state['total_score']}", label=total_score_label),
                    gr.update(visible=True),
                    gr.update(visible=True),
                    gr.update(visible=False),  # Ẩn results_table
                    gr.update(visible=False),  # Ẩn download_button
                    state
                )
            else:
                return (
                    gr.update(value=state["filtered_questions"][current_question_index-1]),  # Keep current question visible
                    gr.update(visible=False),  # Hide user input
                    gr.update(visible=False),  # Hide submit button
                    gr.update(visible=True),  # Show next button
                    gr.update(value=quiz_result, visible=True),  # Show response
                    gr.update(value=f"{state['total_score']}", label=total_score_label),
                    gr.update(visible=False),
                    gr.update(visible=False),
                    gr.update(visible=False),
        ...
        next_button.click(
            next_question,
            inputs=[state],
            outputs=[question_box, user_input, submit_button, next_button, response_box, state],
        )
