...
from createRecordS_checkTenantAccessToken_funct import create_many_records_with_checkTenantAccessToken
import config
import re
...
async def quiz_logic(questions, checking_answers, explain_answers, questions_mc, questions_essay, user_answer, filtered_questions, state):
    ...
    if user_answer:
        ...
        results.append({
            ...
            'topic': state["filtered_topics"][current_question_index],  # Use the specific topic for this question
        })
    ...
    if current_question_index >= num_questions:
        ...
        try:
            ...
            # Có thể thử lưu vào một vị trí khác hoặc với tên file khác
        
        # Send data to Larkbase immediately after quiz completion
        send_results_to_larkbase(df)
...
def send_results_to_larkbase(df):
    records_fields_json = {
        "records": [
            {
                "fields": {
                    ...
                    "user_feedback": row.get('user_feedback', '')  # Use empty string if no feedback yet
                }
    ...
    print(records_fields_json_str)
    
    try:
        create_many_records_with_checkTenantAccessToken(
            config.APP_BASE_TOKEN,
            config.BASE_TABLE_ID,
            json.loads(records_fields_json_str)
        )
...
def save_feedback(feedback, state):
    ...
    # Update Larkbase with feedback
    update_larkbase_with_feedback(df)
...
def update_larkbase_with_feedback(df):
    records_fields_json = {
        "records": [
            {
                "fields": {
    ...
    try:
        # Assuming you have a function to update records in Larkbase
        update_records_in_larkbase(
            config.APP_BASE_TOKEN,
            config.BASE_TABLE_ID,
            json.loads(records_fields_json_str)
        )
        logger.info("Feedback successfully updated in Larkbase")
    except Exception as e:
        logger.error(f"Error updating feedback in Larkbase: {str(e)}")
...
def create_quiz_interface(file_path, sheet_name_mc, sheet_name_essay, output_dir):
    ...
    with gr.Blocks() as demo:
        ...
        with gr.Column(elem_classes="quiz-content"):
            ...
            user_feedback = gr.Textbox(label="Your feedback", visible=False)
            save_feedback_button = gr.Button("LET'S SUMBIT TO SAVE YOUR RESULTS! Thank you!", visible=False)
        ...
        def start_quiz(user_name_input, topic_selector_input, num_questions_input, state):
            ...
            if not user_name_input or not topic_selector_input or not num_questions_input:
                return (
                    ...
                    gr.update(visible=False),
                    gr.update(visible=False),
                    gr.update(value=f"{state['total_score']}", visible=True, label=f"Total Score - {user_name_input} - {', '.join(topic_selector_input)} - {len(state['filtered_questions'])} questions"),
                    gr.update(visible=False),
                    gr.update(visible=False),
                    gr.update(visible=False),
                    gr.update(visible=False),
            ...
            file_safe_user_name = re.sub(r'[^\w\-_\. ]', '_', user_name_input)
            file_safe_topic = re.sub(r'[^\w\-_\. ]', '_', "_".join(topic_selector_input))
            ...
            mc_questions = [(q, t) for q, t in zip(questions_mc, topics_mc) if t in topic_selector_input]
            essay_questions = [(q, t) for q, t in zip(questions_essay, topics_essay) if t in topic_selector_input]
            ...
            state["filtered_questions"] = [q for q, _ in selected_essay + selected_mc]
            state["filtered_topics"] = [t for _, t in selected_essay + selected_mc]
            combined = selected_essay + selected_mc
            random.shuffle(combined)
            state["filtered_questions"], state["filtered_topics"] = zip(*combined) if combined else ([], [])
            ...
            state["filtered_topics"] = state["filtered_topics"][:num_questions]
